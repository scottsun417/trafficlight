...
def infer_on_dataset(img_dir, label_dir, model_path):
    ...
    for file_name in files:
        ...
        infer.input_image_size = [image.size[1], image.size[0]]
        if infer.model_image_size != (None, None):
            assert infer.model_image_size[0] % 32 == 0, 'Multiples of 32 required'
            assert infer.model_image_size[1] % 32 == 0, 'Multiples of 32 required'
            boxed_image = letterbox_image(image, tuple(reversed(infer.model_image_size)))
        else:
            new_image_size = (image.width - (image.width % 32),
                              image.height - (image.height % 32))
            boxed_image = letterbox_image(image, new_image_size)
        ...
        image_data /= 255.
        image_data = np.expand_dims(image_data, 0)  # Add batch dimension.
        result = infer._inference({infer.input_image_key: image_data})
...
def infer_on_one_img(img_path, model_path):
    infer = ObjectDetectionService('', model_path)
    image = Image.open(img_path)
    result = infer._inference({infer.input_image_key: image})
    print(result)


if __name__ == '__main__':
    # img_dir = r'/home/ma-user/work/case_dev/yolov3/mask_detect_VOC/test'
    # label_dir = r'/home/ma-user/work/case_dev/yolov3/mask_detect_VOC/test'
    # model_path = r'/home/ma-user/work/case_dev/yolov3/logs/trained_weights_final.h5'
    # infer_on_dataset(img_dir, label_dir, model_path)

    img_path = r'/home/ma-user/work/competition/debug/img_4.jpg'
    model_path = r'/home/ma-user/work/competition/model'
